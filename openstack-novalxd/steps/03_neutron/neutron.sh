...
openstack security-group-rule-create --protocol tcp --port-range-min 22 \
          --port-range-max 22 --direction ingress default >> $NEUTRON_LOG 2>&1
openstack security-group-rule-create --protocol icmp --direction ingress default >> $NEUTRON_LOG 2>&1
